...
def simplify_structure(dataset):
    return {
        "place": dataset['properties']['place'],
        'magnitude': dataset['properties']['mag'],
        'date': str(datetime.date.fromtimestamp(dataset['properties']['time'] / 1000))
    }

# TODO: transform the largest events into a simpler structure
simplified_data = list(map(simplify_structure, results))
pprint.pp(simplified_data)